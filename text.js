...
const blog_key = blog_wallet.n
// const blog_addr = "vVvN5qZ-KSno5hpsK_t2B-6gq2XGDENX3_ZE1HzzGs4"

async function main() {
  const tx = await arweave.createTransaction({ data: '<h1>Hello World</h1>', tags: [new Tag('Content-Type', 'text/html')] }, blog_wallet)
  // tx.addTag('Content-Type', 'text/html')
  await arweave.transactions.sign(tx, blog_wallet)

  console.log(tx)
  console.log(tx.id)

  const res = await fetchTransaction(tx.id)
  console.log(res)
